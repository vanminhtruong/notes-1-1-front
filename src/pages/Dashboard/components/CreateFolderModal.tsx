...
const CreateFolderModal = ({ isOpen, onClose, onSubmit }: CreateFolderModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({ name: name.trim(), color, icon: icon || 'folder' });
      setName('');
      setColor('blue');
      setIcon('folder');
      onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm-down:p-3 xs-down:p-2.5 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 rounded-2xl md-down:rounded-xl shadow-2xl w-full max-w-md md-down:max-w-sm xs-down:max-w-[92%] max-h-[90vh] flex flex-col overflow-hidden transform transition-all">
        ...
        <div className="flex-1 overflow-y-auto p-6 md-down:p-5 sm-down:p-4 xs-down:p-3.5">
          <form onSubmit={handleSubmit} id="create-folder-form" className="space-y-4 md-down:space-y-3.5 sm-down:space-y-3 xs-down:space-y-3">
            {/* Folder Name */}
            <div>
